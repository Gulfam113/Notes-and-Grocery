...
function Notes() {
    ...
    // Load notes from sessionStorage when the component mounts
    useEffect(() => {
        const storedNotes = sessionStorage.getItem('notes');
        if (storedNotes) {
            setListOfNotes(JSON.parse(storedNotes));
        }
    }, []);

    // Save notes to sessionStorage whenever the listOfNotes changes
    useEffect(() => {
        sessionStorage.setItem('notes', JSON.stringify(listOfNotes));
    }, [listOfNotes]);
        ...
        <div className='h-screen'>
            ...
            <div className='flex justify-center'>
                <button
                    ...
                    className="align-middle block m-3 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                    type="button"
                >
            ...
            {/* Main modal */}
            {modelShow && (
                <div id="authentication-modal" tabIndex="-1" className="flex justify-center overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full backdrop-blur-sm">
            ...
            {listOfNotes.length === 0 ? (
                ...
                <div className="flex flex-wrap justify-center">
                    {listOfNotes.map((note) => (
                        <div key={note.id} className="shadow-sm w-full md:w-1/2 lg:w-1/3 xl:w-1/4 p-4 m-2 bg-white border border-gray-200 rounded-lg  dark:bg-gray-800 dark:border-gray-700">
                            {/* <svg className="w-7 h-7 text-gray-500 dark:text-gray-400 mb-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M18 5h-.7c.229-.467.349-.98.351-1.5a3.5 3.5 0 0 0-3.5-3.5c-1.717 0-3.215 1.2-4.331 2.481C8.4.842 6.949 0 5.5 0A3.5 3.5 0 0 0 2 3.5c.003.52.123 1.033.351 1.5H2a2 2 0 0 0-2 2v3a1 1 0 0 0 1 1h18a1 1 0 0 0 1-1V7a2 2 0 0 0-2-2ZM8.058 5H5.5a1.5 1.5 0 0 1 0-3c.9 0 2 .754 3.092 2.122-.219.337-.392.635-.534.878Zm6.1 0h-3.742c.933-1.368 2.371-3 3.739-3a1.5 1.5 0 0 1 0 3h.003ZM11 13H9v7h2v-7Zm-4 0H5v7h2v-7Zm10 1h-2v6h2v-6Zm-14 0H3v6h2v-6Z" />
                            </svg> */}
                            <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-black">{note.title}</h5>
                            <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">{note.description}</p>
                            <div className='flex justify-end'>
                                <button
                                    onClick={() => handleEdit(note.id)}
                                    // className="mr-2 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-3 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                                    className="mr-2   "
                                >
                                    <CiEdit size={32} className='text-blue-800' />
                                </button>
                                <button
                                    ...
                                    className="mr-2 dark:text-blue  "

                                >
                                    <RiDeleteBin6Line size={32} className="text-red-800" />
